...
function calculateTotalPrice(products: Product[]): number {
    return products.reduce((total, product) => {
        if(product.discount !== undefined && (product.discount < 0 || product.discount > 100)) {
            throw new Error(`Invalid discount for ${product.name}: ${product.discount}. Discount must be between 0 and 100.`);
        }
        
        const productTotal = product.price * product.quantity;
        const discount = product.discount ? (productTotal * product.discount) / 100 : 0;
